...
public class SmartEdgeHandler extends FlatEdgeHandler {
    ...
    protected void handleConflict(Node<MavenContext> node, List<Edge> edges) {
        ...
        String artifactIdentifier = TreeHelper.getArtifactId(context.getDependencyNode().getRelatedArtifact());

        // Find the node containing the replacement artifact Maven is actually going to use
        Node<MavenContext> replacement = findIncludedNode(node.getRoot(), artifactIdentifier);
